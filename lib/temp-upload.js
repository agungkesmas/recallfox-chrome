...
//
// v3.24.19: catatan gofile.io → temp.sh — alur MANUAL berbeda dengan upload
// otomatis: di alur MANUAL user yang mengunggah lewat browsernya dan menempel
// URL halaman unduh, jadi "URL = halaman HTML" bukan masalah (item vault
// manual TIDAK pernah fetch isi file — body kosong, size 0).
//
...
// v3.24.18: tujuan ketiga — MANUAL. User upload sendiri di situs luar (klik,
...
// dihapus otomatis dari vault oleh cleanupExpiredTempItems yang sama.
// (Paritas 1:1 dengan PWA v1.21.0 yang sudah E2E 7/7.)
// v3.24.19: gofile.io KELUAR dari daftar (laporan user: sudah tidak bisa
...
// halaman HTML, bukan file mentah (lihat audit di atas).
// v3.24.24: gofile.io KEMBALI ke daftar default (permintaan user — dipakai
// untuk workspace AI agent; riset ulang 2026-09-12: situs hidup, multi-node
// check HTTP 200; laporan lama sudah usang). Alur MANUAL tetap: item manual
// tidak pernah fetch isi file, jadi halaman unduh gofile bukan masalah.
export const TEMP_HOST_MANUAL = 'manual';
export const MANUAL_TEMP_DURATION = '72h';
// v3.24.24: daftar situs default hasil riset + uji upload curl LIVE 2026-09-12
// (18 kandidat → 10 lolos): gofile (populer, tanpa akun, halaman unduh),
// litterbox (raw, 1GB, 1–72 jam, uji OK), tmpfiles (100MB, 7 hari, uji OK),
// filebin (uji OK, bin hilang 6 hari), temp.sh (uji POST /upload OK, hilang
// 3 hari), uguu (uji OK, 128MB, 3 jam), x0.at (uji OK, ≥30 hari), pixeldrain
// (web upload anon OK; API butuh key), storage.to (25GB, tanpa akun), catbox
// (hidup; upload anonim API kadang ditolak "Invalid uploader" → pakai
// litterbox). Tidak dimasukkan karena MATI/gagal uji: transfer.sh (DNS
// hilang), bashupload.com (DNS hilang), fileconvoy (mati), 0x0.st (flaky),
// file.io (sekali unduh), krakenfiles (uji 2 langkah tidak lolos).
...
// storage lokal key 'recallfox_manual_sites' (addon: browser.storage.local,
// PWA: localStorage) dan divalidasi oleh sanitizeManualSites() di bawah.
...
// v3.24.19: pengelolaan daftar situs oleh user.
// MANUAL_SITES_MAX — batas jumlah situs (popup terbatas ruang; cegah overflow).
