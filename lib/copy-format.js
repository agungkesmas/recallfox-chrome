...
export async function writeScreenshotToClipboard(dataUrl, textPlain, textHtml) {
  ...
  // v3.20.22: Strategi 4 — background relay ke content script tab aktif.
  // Dipakai kalau strategi 1/2/3 semua gagal (typical di iframe popout sidebar Chrome).
  // Background kirim COPY_IMAGE ke content script di tab aktif — content script
  // jalan di halaman web context (focused) → clipboard.write works.
  if (typeof browser !== 'undefined' && browser.runtime?.sendMessage && dataUrl) {
    try {
      const res = await browser.runtime.sendMessage({
        type: 'COPY_IMAGE',
        dataUrl,
        textPlain: textPlain || null,
      ...
      if (res?.ok) {
        return {
          ok: true,
          message: '✓ Gambar + keterangan tersalin via tab aktif',
          fallback: 'bg_relay'
        };
      }
      console.warn('[RecallFox] writeScreenshot strategi 4 (bg relay) failed:', res?.error);
    } catch (e) {
      console.warn('[RecallFox] writeScreenshot strategi 4 exception:', e.message);
    }
  }
...
export async function writeImageOnlyToClipboard(dataUrl) {
  ...
  // v3.20.22: Strategi D — background relay ke content script tab aktif.
  // Dipakai kalau A/B/C semua gagal (typical di iframe popout sidebar Chrome
  // karena navigator.clipboard.write butuh document focused + user gesture
  // yang tidak reliable di iframe cross-origin).
  // Background kirim COPY_IMAGE ke content script di tab aktif — content script
  // jalan di halaman web context (focused) → clipboard.write works.
  if (typeof browser !== 'undefined' && browser.runtime?.sendMessage) {
    try {
      ...
      if (res?.ok) {
        return {
          ok: true,
          message: '✓ Gambar tersalin via tab aktif',
          fallback: 'bg_relay'
        };
      }
      console.warn('[RecallFox] writeImageOnly strategi D (bg relay) failed:', res?.error);
    } catch (e) {
      console.warn('[RecallFox] writeImageOnly strategi D exception:', e.message);
    }
  }
