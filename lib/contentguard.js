// lib/contentguard.js — Content Guardian: Mode Fokus Allowlist YouTube
// RecallFox v0.8.20 → v3.21.0 (Mode Fokus) → v3.24.25 (fitur X/Twitter DIHAPUS)
//
// v3.24.25: SEMUA bagian X (Twitter) dihapus — DEFAULT_BLOCKED_X_ACCOUNTS,
// DEFAULT_CHINA_X_ACCOUNTS, DEFAULT_CHINA_X_SEARCHES, isXHome, cabang X di
// detectSearchQuery, tipe blocklist 'account'/'x_post_url', dan settings X.
// Addon tidak lagi memfilter/memblokir apa pun di x.com/twitter.com.
...
// ===== v0.8.24: Helper - cek apakah URL adalah hasil search YouTube =====
...
// Blocklist entry: { type: 'title'|'channel'|'keyword'|'exact_title', value: '...' }
...
//   (v3.24.25: tipe 'account' & 'x_post_url' khusus X dihapus — entri lama
//    diperlakukan sebagai keyword.)
...
// ============================================================================
// Lihat: prompt-agent-rombak-firefox.md (instruksi lengkap)
//
// Konsep: ganti mekanisme inti dari BLACKLIST keyword negatif (650 keyword)
// menjadi ALLOWLIST topik. Saat Mode Fokus aktif dengan profil P, video
// YouTube TAMPIL hanya jika cocok dengan profil P. Semua video lain
// disembunyikan total. Pencarian juga dikunci (lihat §4.6 instruksi).
//
// Skema: contentGuardTopicProfiles (di vault.settings)
//   { profiles: [{id, emoji, name, topics[], channels[], strictWatch}], activeProfileId }
//
// Field `enabled` per profil TIDAK ada — model "satu saklar": yang ON
// ditentukan oleh `activeProfileId` (radio, bukan toggle independen).
// ============================================================================

/**
 * 2 profil bawaan, di-seed saat pertama kali Mode Fokus diaktifkan
 * (di initContentGuardDefaults background.js). User bisa tambah/hapus.
 */
export const DEFAULT_TOPIC_PROFILES = [
  {
    id: 'p_focus_self',
    emoji: '👤',
    name: 'Fokus Belajar',
  ...
  },
  {
    ...
    emoji: '🧒',
    name: 'Fokus Anak',
    topics: ['cerita nabi', 'lagu anak', 'dongeng'],
    channels: ['Nussa Official', 'Cocomelon'],
    strictWatch: true
  }
];

/**
 * Buat ID profil baru (timestamp-based, unik).
 * Dipakai saat user klik "Tambah Profil Baru" di Settings/Popup.
 */
...
/**
 * Cache normalizeText untuk topik & channel profil aktif.
 * Dipanggil saat load settings / ganti profil. Hindari normalizeText di
 * dalam loop match (requirement §5.1: cache sekali saat load).
 *
 * @param {Object} profile — profil aktif ({ topics: [], channels: [] })
 * @returns {{ topics: Set<string>, channels: Set<string> }} — Set berisi teks sudah dinormalisasi
 */
export function buildProfileMatchCache(profile) {
  if (!profile) return { topics: new Set(), channels: new Set() };
...
/**
 * Cek apakah video (judul + nama channel) cocok dengan profil aktif.
 *
 * Aturan match (OR) — video TAMPIL jika:
 *   - judul mengandung salah satu topik, ATAU
 *   - nama channel mengandung salah satu topik, ATAU
 *   - channel cocok salah satu channel whitelist
 *
 * Match literal (substring, case-insensitive, dengan normalisasi anti-leet).
 * Tidak ada sinonim/sihir — user menambah varian sebagai topik terpisah.
 *
 * @param {string} title — judul video (sudah diambil dari DOM)
 * @param {string} channel — nama channel
 * @param {{ topics: Set<string>, channels: Set<string> }} cache — hasil buildProfileMatchCache
 * @returns {boolean} — true = cocok (tampil), false = tidak cocok (sembunyikan)
 */
export function matchesActiveProfile(title, channel, cache) {
  if (!cache) return false;
  const normTitle = normalizeText(title);
  const normChannel = normalizeText(channel);
  // Cek topik di judul
  for (const topic of cache.topics) {
    if (topic && normTitle.includes(topic)) return true;
  }
  // Cek topik di nama channel
  for (const topic of cache.topics) {
    if (topic && normChannel.includes(topic)) return true;
  }
  // Cek channel whitelist
  for (const ch of cache.channels) {
    if (ch && normChannel.includes(ch)) return true;
  }
  return false;
}

/**
 * Cek apakah query pencarian cocok dengan profil aktif (untuk Kunci Pencarian).
 *
 * Query COCOK jika mengandung salah satu topik (match literal, setelah
 * lowercasing + normalisasi anti-leet). Query yang dihasilkan sistem (dari
 * kartu halaman Search Lock) sudah pasti cocok → tidak ter-block.
 *
 * @param {string} query — search query user
 * @param {Object} profile — profil aktif
 * @returns {boolean} — true = query cocok (tampil), false = query di luar topik (kunci)
 */
export function matchesProfileSearchQuery(query, profile) {
  if (!query || !profile || !Array.isArray(profile.topics) || profile.topics.length === 0) {
    return false;
  }
  const normQuery = normalizeText(query);
  for (const t of profile.topics) {
    const n = normalizeText(t);
    if (n && normQuery.includes(n)) return true;
  }
  return false;
}

/**
 * Ambil profil aktif dari settings (atau null kalau tidak ada profil aktif).
 *
 * @param {Object} topicProfiles — { profiles: [], activeProfileId: string|null }
 * @returns {Object|null} — profil aktif atau null
 */
export function getActiveProfile(topicProfiles) {
  if (!topicProfiles || !Array.isArray(topicProfiles.profiles)) return null;
  if (!topicProfiles.activeProfileId) return null;
  return topicProfiles.profiles.find(p => p.id === topicProfiles.activeProfileId) || null;
}

/**
 * Cek apakah profil aktif valid untuk filtering (punya topik atau channel).
 * Profil kosong = tidak menyaring apa pun (semua tampil) + Search Lock nonaktif.
 *
 * @param {Object|null} profile — profil aktif
 * @returns {boolean}
 */
export function isProfileFiltering(profile) {
...
/**
 * Seed 2 profil bawaan kalau contentGuardTopicProfiles belum ada di settings.
 * Dipanggil dari initContentGuardDefaults() di background.js.
 * Jangan menimpa data user yang sudah ada.
 *
 * @param {Object|null} existing — contentGuardTopicProfiles yang sudah ada (atau null)
 * @returns {Object} — contentGuardTopicProfiles (existing atau seed baru)
 */
export function seedDefaultTopicProfiles(existing) {
  if (existing && Array.isArray(existing.profiles)) {
    return existing;
  }
  return {
    profiles: DEFAULT_TOPIC_PROFILES.map(p => ({ ...p, topics: [...p.topics], channels: [...p.channels] })),
    activeProfileId: null  // default OFF — user pilih setelah isi topik
  };
}
