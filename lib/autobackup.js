...
// killing any setInterval callbacks. Auto-backup (default 6 hours) would
// essentially NEVER fire on schedule in Chrome. Use browser.alarms API
// which can wake up the SW even from sleep. Firefox MV3 SW also benefits
// (already the pattern used by Supabase realtime + auto-discard).
const BACKUP_ALARM_NAME = 'rf-auto-backup';
...
export async function startBackupInterval() {
  ...
  const periodInMinutes = Math.max(1, hours * 60);  // min 1 minute (browser.alarms limit)
  console.log(`[RecallFox] Auto-backup alarm started: every ${hours} hour(s) (${hours === 6 ? '4x/day' : hours === 12 ? '2x/day' : hours === 24 ? '1x/day' : '24x/day'})`);
  // v3.20.5: Use browser.alarms (reliable in Chrome MV3 SW).
  // The alarm handler in background.js calls manualBackupNow() when this alarm fires.
  try {
    await browser.alarms.create(BACKUP_ALARM_NAME, { periodInMinutes });
  } catch (e) {
    console.warn('[RecallFox] Failed to create backup alarm, fallback to setInterval:', e.message);
    // Fallback: setInterval (less reliable, but better than nothing)
    const intervalMs = hours * 60 * 60 * 1000;
    backupTimer = setInterval(() => manualBackupNow().catch(() => {}), intervalMs);
  }
  // Run once on startup (after 10s delay to let addon settle).
  // This is a best-effort one-shot — only fires if SW happens to be alive,
  // which it is right after onInstalled/onStartup.
  setTimeout(() => manualBackupNow().catch(() => {}), 10000);
}

// v3.20.5: Exported for background.js alarm handler to call.
export async function handleBackupAlarm() {
  try {
    await manualBackupNow();
  } catch (e) {
    console.warn('[RecallFox] Auto-backup alarm handler error:', e.message);
  }
}
...
// v3.10.2 (Issue 1 fix): Tambahkan `export` — sebelumnya tidak di-export, sehingga
// `await import('./lib/autobackup.js')` di background.js return undefined dan
// tombol "Full Backup ke GDrive" gagal dengan "buildBackupPayload is not a function".
...
//
// Chrome MV3 note: URL.createObjectURL is NOT available in service worker
// context. We detect this and fall back to a base64 data: URL. Firefox SW
// supports URL.createObjectURL natively, so we use the more efficient blob
// URL there.
async function writeToDisk(filename, payload, saveAs = false) {
  ...
  if (typeof URL !== 'undefined' && typeof URL.createObjectURL === 'function') {
    // Firefox SW / popup context — use blob URL (efficient, no base64 overhead)
    const blob = new Blob([json], { type: 'application/json' });
    url = URL.createObjectURL(blob);
    isBlobUrl = true;
  } else {
    // Chrome MV3 SW — URL.createObjectURL not available; use data: URL instead.
    // btoa is available in SW. For UTF-8 safety, encode UTF-8 bytes first.
  ...
  try {
    ...
    // Revoke blob URL after a delay (download needs it to complete).
    // data: URLs don't need revocation.
    if (isBlobUrl) setTimeout(() => URL.revokeObjectURL(url), 30000);
    return downloadId;
  } catch (e) {
    if (isBlobUrl) setTimeout(() => URL.revokeObjectURL(url), 5000);
    throw e;
  }
}
