...
// v3.22.6: + getScreenshotBlob/setScreenshotBlob — import() dinamis dilarang di ServiceWorker
...
// v3.22.6: diekspor — sebelumnya private sehingga guard mod._getDeleteRegistry di
// storage.js selalu false (filter delete-registry tidak pernah aktif di browser mana pun).
export async function _getDeleteRegistry() {
  try {
...
export async function directUpsertVaultItem(item) {
  ...
  try {
    ...
    // v3.24.12: File SEMENTARA (source.tempHost) TIDAK di-upload ke Storage —
    //   file aslinya hidup di host sementara (litterbox, source.tempUrl).
    //   Yang disinkron ke cloud hanya metadata + URL. Item dihapus otomatis
    //   dari vault (lokal + cloud) saat kedaluwarsa oleh cleanupExpiredTempItems.
    if (item.type === 'file' && !(item.source && item.source.tempHost)) {
      const fileRes = await _uploadFileDocument(item, userId);
      if (!fileRes.ok) {
