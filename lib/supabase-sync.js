...
import { getVault, getNotes, saveVault, saveNotes, getSettings, saveSettings, getAllScreenshotBlobKeys, getScreenshotBlob, setScreenshotBlob } from './storage.js';
...
const DOCUMENTS_BUCKET = 'documents';  // v3.20.36 port dari Firefox: bucket untuk file teks upload
  ...
  try {
    ...
    for (const item of items) {
      try {
        ...
        // v3.20.36 port dari Firefox: Kalau file document, upload ke Storage bucket 'documents'
        if (item.type === 'file' && item.body) {
          const fileRes = await _uploadFileDocument(item, userId);
          if (fileRes.ok) {
            console.log('[RecallFox/Supabase] File document uploaded:', item.id);
          } else {
            stats.errors.push({
              id: item.id,
              type: 'file',
              title: item.title,
              error: 'storage: ' + fileRes.error
            });
...
// ============== v3.20.36 port dari Firefox: UPLOAD FILE DOCUMENT ==============
// Upload file teks (md/txt/json/html/csv/yaml) ke Storage bucket 'documents'.
// Path: user-<uuid>/<item-id>.<ext>
// Set upload, PATCH gdrive_file_url di vault_items + update lokal vault.
//
...
async function _uploadFileDocument(item, userId) {
  try {
    if (!item.body) return { ok: false, error: 'empty_file_body' };
        ...
        browser.storage.local.set({ 'recallfox_last_sync_error': JSON.stringify({
          ...
          hint: upRes.error === 'not_logged_in'
            ...
            : (upRes.error === 'http_403'
              ? 'RLS policy block - jalankan SQL recallfox-v3.20.38-documents-bucket.sql'
...
export async function directUpsertVaultItem(item) {
  ...
  try {
    ...
    // v3.20.36 port dari Firefox: Upload file document ke Storage bucket 'documents'
    if (item.type === 'file') {
      const fileRes = await _uploadFileDocument(item, userId);
      if (!fileRes.ok) {
        console.warn('[RecallFox/Supabase] File document upload failed:', item.id, fileRes.error);
...
export async function directDeleteVaultItem(itemId) {
  ...
  try {
    ...
    // v3.20.36 port dari Firefox: Hapus file document dari bucket 'documents' kalau type='file'
    try {
      const itemInfo = itemRes.data?.[0];
      if (itemInfo && itemInfo.gdrive_file_id && itemInfo.type === 'file') {
        const delFileRes = await deleteFile(DOCUMENTS_BUCKET, itemInfo.gdrive_file_id);
        if (!delFileRes.ok) {
          console.warn('[RecallFox/Supabase] File document delete failed:', itemId, delFileRes.error);
        }
      }
    } catch (e) {
      console.warn('[RecallFox/Supabase] File document cleanup failed:', itemId, e.message);
    }
...
// v3.20.38-dev-chrome: Per-ID lock map untuk serialisasi realtime events.
...
//   Port dari Firefox v3.20.39-dev.
      ...
      _realtimeChannel.onmessage = (ev) => {
        try {
          ...
          if (msg.event === 'INSERT' || msg.event === 'UPDATE' || msg.event === 'DELETE') {
            // v3.20.38-dev-chrome: Serialize per-ID supaya 3 events (INSERT+UPDATE+UPDATE)
...
export async function pullFromSupabaseV33() {
  ...
  try {
    ...
    console.log('[RecallFox/Supabase] pullV33: starting pull for userId:', userId);
    const itemsRes = await selectRows(VAULT_TABLE, {
      select: '*',
      filter: `user_id=eq.${userId}&deleted_at=is.null`,
      order: 'updated_at.desc'
    });
    console.log('[RecallFox/Supabase] pullV33: selectRows result:', { ok: itemsRes.ok, error: itemsRes.error, dataLen: itemsRes.data?.length });
    if (!itemsRes.ok) {
      console.error('[RecallFox/Supabase] pullV33: selectRows FAILED:', itemsRes.error);
      return { ok: false, error: itemsRes.error || 'pull_failed', stats };
    }
    if (itemsRes.ok && itemsRes.data) {
      const vault = await getVault();
      ...
      // v3.20.38-dev-chrome: Dedup localItems by ID SEBELUM merge.
      ...
      //   Port dari Firefox v3.20.39-dev. Fix BUG 3 (list duplikat).
