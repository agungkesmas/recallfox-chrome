...
// v3.22.6 (Chrome MV3): import() dinamis DILARANG di ServiceWorkerGlobalScope
// (HTML spec — w3c/ServiceWorker#1356). Lazy import GDriveSync diganti static
// import. Siklus storage <-> gdrive-sync <-> supabase-sync aman karena semua
// pemakaian antar modul hanya di dalam fungsi (bukan saat evaluasi modul).
import { notify as _gdriveNotify, uploadScreenshot as _gdriveUploadScreenshot } from './gdrive-sync.js';
import { addToDeleteRegistry, _getDeleteRegistry, directUpsertVaultItem, directDeleteVaultItem, directUpsertNote, directDeleteNote } from './supabase-sync.js';

// v3.22.6: bridge object untuk mempertahankan pola lama import('./supabase-sync.js').then(mod => ...)
// tanpa dynamic import (dilarang di ServiceWorker). Semua export function declaration
// di supabase-sync.js sehingga binding tersedia sejak instantiasi modul.
const SUPA_MOD = { directUpsertVaultItem, directDeleteVaultItem, directUpsertNote, directDeleteNote };
...
// v3.22.0 (Fase 2): storage.local key prefix untuk blob file binary (PDF/Office/gambar).
// Pola sama dengan rf_shot_ — vault JSON TIDAK boleh menyimpan data binary
// (fix bug v3.11.33: vault JSON membengkak → storage.local lambat/crash).
const FILE_PREFIX = 'rf_file_';
...
export async function addItem(item, opts = {}) {
  const vault = await getVault();
  ...
  // v3.22.0 (Fase 2): File binary (PDF/Office/gambar) — blob dilewatkan lewat
  // opts.fileBlob, disimpan di key terpisah rf_file_{id}. Vault JSON hanya
  // simpan metadata (body dibiarkan kosong).
  let fileBlob = null;
  if (type === 'file' && opts && opts.fileBlob) {
    fileBlob = opts.fileBlob;
  }
  const isBinaryFile = type === 'file' && !!(item.source && item.source.isBinary);
  if (isBinaryFile && fileBlob) {
    // body tidak boleh berisi data binary — paksa kosong.
    item.body = '';
  }
  ...
  // v3.22.0 (Fase 2): Simpan blob file binary di key terpisah (data URL —
  // format yang sama dengan rf_shot_, aman untuk Chrome & Firefox).
  let fileDataUrl = null;
  if (fileBlob) {
    try {
      fileDataUrl = await _blobToDataUrl(fileBlob);
      await browser.storage.local.set({ [`${FILE_PREFIX}${newItem.id}`]: fileDataUrl });
    } catch (e) {
      console.warn('[RecallFox] Failed to store file blob:', e);
    }
  }
  ...
  if (type === 'screenshot' && screenshotBlob) {
    ...
    Promise.resolve(SUPA_MOD).then(mod => {
    ...
    Promise.resolve(SUPA_MOD).then(mod => {
      if (mod.directUpsertVaultItem) {
        // v3.22.0 (Fase 2): binary file — lampirkan dataUrl di properti
        // sementara fileDataUrl (BUKAN masuk vault JSON), mirror pola
        // screenshot di atas. _uploadFileDocument akan memakainya.
        const itemForCloud = (isBinaryFile && fileDataUrl) ? { ...newItem, fileDataUrl } : newItem;
        mod.directUpsertVaultItem(itemForCloud).catch(e => {
...
export async function deleteItem(id) {
  ...
  // v3.22.0 (Fase 2): Kalau file binary, hapus juga blob rf_file_{id}.
  if (removed?.type === 'file' && removed?.source?.isBinary) {
...
// ===== v3.22.0 (Fase 2): blob file binary (rf_file_{id}) =====
// Data URL — format sama dengan rf_shot_ (aman Chrome & Firefox).

// Helper internal: Blob → data URL. FileReader tersedia di semua context
// extension; fallback manual base64 untuk environment non-browser (test Node).
function _blobToDataUrl(blob) {
  return new Promise((resolve, reject) => {
    ...
    // Fallback non-browser (mis. test Node): arrayBuffer + btoa
...
// Ambil data URL blob file binary (null kalau tidak ada / bukan binary)
export async function getFileDataUrl(id) {
  const key = `${FILE_PREFIX}${id}`;
  const data = await browser.storage.local.get(key);
  return data[key] || null;
}

// Set / replace data URL blob file binary
export async function setFileDataUrl(id, dataUrl) {
  await browser.storage.local.set({ [`${FILE_PREFIX}${id}`]: dataUrl });
}

// Hapus blob file binary dari storage.local
export async function deleteFileBlob(id) {
  try {
    await browser.storage.local.remove(`${FILE_PREFIX}${id}`);
  } catch (e) {
    console.warn('[RecallFox] Failed to remove file blob:', e);
  }
}
...
export async function deleteBundle(id) {
  ...
  try {
    // v3.22.6: static import — import() dinamis dilarang di ServiceWorker
    if (addToDeleteRegistry) {
...
export async function mergeSyncIntoLocal() {
  ...
  try {
    // v3.22.6: static import — import() dinamis dilarang di ServiceWorker
    if (_getDeleteRegistry) {
      deleteReg = await _getDeleteRegistry();
    }
  } catch (e) { /* ignore */ }
...
export function onSyncChange(callback) {
  // v3.22.6: Chrome TIDAK punya storage.sync.onChanged (hanya Firefox yang punya
  // per-area onChanged). Guard + fallback ke storage.onChanged dgn filter area
  // 'sync' — semantik identik, aman di kedua browser.
  ...
  try {
    if (browser.storage.sync && browser.storage.sync.onChanged) {
      browser.storage.sync.onChanged.addListener(handler);
    } else if (browser.storage.onChanged) {
      browser.storage.onChanged.addListener(handler);
    }
  } catch (e) {
    console.warn('[RecallFox] onSyncChange: tidak bisa daftar listener:', e.message);
  }
}
...
// Schema per note (v3.21.21 — P1-P4 + dueAt + labels + done + subtasks + recurring):
//   {
...
//     subtasks:  Array<{id,text,done,doneAt}>, // v3.21.20: checklist subtasks ala Todoist (max 20)
//     recurring: null|{freq,byDay,interval}, // v3.21.21: tiap hari/minggu/bulan/tahun/senin
...
export async function getNotes() {
  ...
  // v3.21.21: migrasi recurring + subtasks + done/doneAt/priority/dueAt/labels untuk notes lama
  notes = notes.map(n => {
    const base = { done: false, doneAt: null, priority: 4, dueAt: null, labels: [], subtasks: [], recurring: null, ...n };
    // sanitize subtasks — only keep valid entries, max 20
    if (!Array.isArray(base.subtasks)) base.subtasks = [];
    base.subtasks = base.subtasks.filter(s=> s && typeof s.text==='string' && s.text.trim()).slice(0,20).map(s=> ({
      id: s.id || 's_'+Math.random().toString(36).slice(2,8),
      text: String(s.text).slice(0,200).trim(),
      done: !!s.done,
      doneAt: s.doneAt || null
    }));
    if (![1,2,3,4].includes(base.priority)) base.priority=4;
    if (!Array.isArray(base.labels)) base.labels=[];
    if (base.recurring && typeof base.recurring !== 'object') base.recurring=null;
    return base;
  });
...
export async function addNote(body = '', opts = {}) {
  ...
  const note = {
    ...
    subtasks: Array.isArray(opts.subtasks) ? opts.subtasks.slice(0,20).map(s=> ({ id: s.id || 's_'+Math.random().toString(36).slice(2,8), text: String(s.text||'').slice(0,200).trim(), done: !!s.done, doneAt: s.doneAt||null })).filter(s=>s.text) : [],
