...
export async function addItem(item) {
  ...
  // v3.20.39 (port dari Firefox): Defense-in-depth — jangan push kalau ID sudah ada.
  // Cek existing ID. Kalau ada, update (last-write-wins by updatedAt) bukan push duplikat.
  ...
  if (type === 'screenshot' && screenshotBlob) {
    ...
    if (directUpsertVaultItem) {
      directUpsertVaultItem(itemForCloud).catch(e => {
    ...
    if (directUpsertVaultItem) {
      directUpsertVaultItem(newItem).catch(e => {
...
export async function updateItem(id, patch) {
  ...
  if (directUpsertVaultItem) {
    directUpsertVaultItem(updated).catch(e => {
...
export async function deleteItem(id) {
  ...
  if (directDeleteVaultItem) {
    directDeleteVaultItem(id).catch(e => {
...
export async function mergeSyncIntoLocal() {
  ...
  // Chrome MV3 note: _getDeleteRegistry is not exported from supabase-sync.js
  // (it's an internal function). The previous dynamic-import block was therefore
  // a no-op — `mod._getDeleteRegistry` was always undefined. We preserve that
  // behavior by leaving deleteReg empty here. To enable actual filtering,
  // export _getDeleteRegistry or use isInDeleteRegistry(b.id) per-bundle.
  let deleteReg = { items: {} };
...
export async function updateNote(id, patch) {
  ...
  // v3.13.3 (A4 fix): Log + simpan error ke storage supaya UI bisa tampilkan.
  if (directUpsertNote) {
    directUpsertNote(notes[idx]).catch(e => {
