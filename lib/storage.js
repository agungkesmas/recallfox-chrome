...
// Chrome MV3: dynamic import() is disallowed on ServiceWorkerGlobalScope.
// Convert all lazy imports to static imports. Circular imports
// (storage ↔ supabase-sync ↔ gdrive-sync) are safe because ES modules
// hoist function declarations — imports are only USED inside functions,
...
import {
  addToDeleteRegistry,
  directDeleteVaultItem,
  directUpsertVaultItem,
  directUpsertNote,
  directDeleteNote,
  _getDeleteRegistry
...
export async function deleteBundle(id) {
  ...
  try {
    ...
    if (directDeleteVaultItem) {
      const res = await directDeleteVaultItem(id);
      if (!res.ok) {
