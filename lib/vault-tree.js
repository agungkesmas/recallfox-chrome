...
export async function aiAutoGroup(items, chatFn, options = {}) {
  ...
  // v3.20.31: User instruction — ide struktur folder yang user ketik sendiri.
  // Di-inject ke prompt sebagai additional guidance. AI harus ikuti instruksi
  // user tapi tetap baca konteks item untuk nama folder yang pas.
  const userInstruction = (typeof options.userInstruction === 'string' && options.userInstruction.trim())
    ? options.userInstruction.trim().slice(0, 1000)
    : '';
...
- Contoh BURUK: "Folder A", "Items", "Stuff", "Things"`;
  ...
  // v3.20.31: Kalau ada userInstruction, regenerate tetap variasi tapi hormati kerangka user.
  const regenerateHint = regenerate > 0
    ? (userInstruction
      ? `\n\nPERHATIAN: Ini percobaan ke-${regenerate + 1} dengan instruksi user yang sama. AI BOLEH:
- Variasikan nama folder (selama masih sesuai kerangka user)
- Variasikan subfolder / struktur nested
- Tambah atau kurangi folder selama masih meng-cover semua item
Tapi kerangka utama instruksi user tetap jadi acuan.`
...
- Output PURE JSON array${regenerateHint}`;
  ...
  // v3.20.31: User instruction block — kalau user ketik ide struktur sendiri.
  // regenerateHint sudah ada di system prompt, jdi di sini cukup tambah instruksi user.
...
- Perbaiki nama folder kalau perlu (mis. user ketik "frontend" → AI bisa ubah jadi "Frontend Frameworks" kalau lebih deskriptif)
