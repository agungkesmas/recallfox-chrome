...
(async function () {
  ...
  // v3.24.5 REWRITE DRAG (paritas notes-cs.js) — mouseup lama memanggil
  // tidy() sehingga hasil drag SELALU dibatalkan (snap balik ke deretan),
  // dan mouseup di atas iframe / di luar window tak pernah diterima →
  // widget "lengket ke kursor ga mau lepas". Kini: Pointer Events +
  // setPointerCapture di header + hasil drop jadi slot khusus dock
  // (pinCustom); klik ganda header = kembali ke deretan (clearCustom).
  function makeDraggable() {
    const hd = shadow.querySelector('.rft-hd');
    const DOCK_KEY = 'tape:' + data.id;
    ...
    try { window.addEventListener('blur', () => { if (dragging) { dragging = false; pid = null; popover.style.transition = ''; } }); } catch (err) {}
