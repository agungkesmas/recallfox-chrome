# RecallFox Chrome v3.20.39 — Fix Definitif: Tombol Upload Tidak Bereaksi

**Release date:** 2026-08-06
**Tag:** `v3.20.39-dev-chrome` (critical bug fix — 4x attempt, now definitively fixed)
...
3. Chrome MV3 popup kadang menolak buka file picker untuk `display:none` input

**Kode lama:** `<input type="file" id="docFileInput" style="display:none" />`

Chrome MV3 popup punya policy ketat tentang user gesture + file input. `display:none` element dianggap "not rendered" → `.click()` tidak buka picker.
...
3. **v3.20.38:** Sama seperti v3.20.37, tidak sadar bahwa `saveScreenshotManualSheet()` membuka sheet sendiri.
...
```html
<!-- SEBELUMNYA (display:none — Chrome MV3 popup kadang tolak .click()) -->
...
**Pattern untuk hidden file input di Chrome Extension popup:**
...
<!-- ❌ SALAH: display:none — Chrome MV3 popup kadang tolak .click() -->
...
3. `display:none` pada file input menyulitkan Chrome MV3 popup untuk trigger `.click()`

**Best practice untuk file upload di Chrome Extension popup:**
